...
static const ibyte c_rlpMaxLengthBytes = 8;
static const ibyte c_rlpDataImmLenStart = 0x80;
static const ibyte c_rlpListStart = 0xc0;

static const ibyte c_rlpDataImmLenCount = c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthBytes;
static const ibyte c_rlpDataIndLenZero = c_rlpDataImmLenStart + c_rlpDataImmLenCount - 1;
static const ibyte c_rlpListImmLenCount = 256 - c_rlpListStart - c_rlpMaxLengthBytes;
static const ibyte c_rlpListIndLenZero = c_rlpListStart + c_rlpListImmLenCount - 1;
...
public:
    ...
    RLP(ibyte const* _b, unsigned _s, Strictness _st = VeryStrict): RLP(bytesConstRef(_b, _s), _st) {}

    /// Construct a node to read RLP data in the string.
    explicit RLP(std::string const& _s, Strictness _st = VeryStrict): RLP(bytesConstRef((ibyte const*)_s.data(), _s.size()), _st) {}
...
private:
    ...
    void pushCount(size_t _count, ibyte _offset);
