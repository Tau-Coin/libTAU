...
public:
    ...
    explicit FixedHash(ibyte const* _bs, ConstructFromPointerType) { memcpy(m_data.data(), _bs, N); }
    ...
    explicit operator bool() const { return std::any_of(m_data.begin(), m_data.end(), [](ibyte _b) { return _b != 0; }); }
    ...
    ibyte& operator[](unsigned _i) { return m_data[_i]; }
    /// @returns a particular byte from the hash.
    ibyte operator[](unsigned _i) const { return m_data[_i]; }
    ...
    ibyte* data() { return m_data.data(); }

    /// @returns a constant byte pointer to the object's data.
    ibyte const* data() const { return m_data.data(); }

    /// @returns begin iterator.
    auto begin() const -> typename std::array<ibyte, N>::const_iterator { return m_data.begin(); }

    /// @returns end iterator.
    auto end() const -> typename std::array<ibyte, N>::const_iterator { return m_data.end(); }

    /// @returns a copy of the object's data as a byte vector.
    bytes asBytes() const { return bytes(data(), data() + N); }

    /// @returns a mutable reference to the object's data as an STL array.
    std::array<ibyte, N>& asArray() { return m_data; }

    /// @returns a constant reference to the object's data as an STL array.
    std::array<ibyte, N> const& asArray() const { return m_data; }
...
private:
    std::array<ibyte, N> m_data;        ///< The binary data.
};
...
public:
    ...
    explicit SecureFixedHash(ibyte const* _d, ConstructFromPointerType _t): FixedHash<T>(_d, _t) {}
    ...
    ibyte const* data() const { return FixedHash<T>::data(); }
