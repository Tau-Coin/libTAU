...
#include "libtorrent/aux_/common_data.h"
...
public:
    ...
    explicit FixedHash(u_byte const* _bs, ConstructFromPointerType) { memcpy(m_data.data(), _bs, N); }

    /// Explicitly construct, copying from a  string.
    explicit FixedHash(std::string const& _s, ConstructFromStringType _t = FromHex, ConstructFromHashType _ht = FailIfDifferent): FixedHash(_t == FromHex ? fromHex(_s, WhenError::Throw) : libtorrent::aux::asBytes(_s), _ht) {}

    /// Convert to arithmetic type.
    operator Arith() const { return fromBigEndian<Arith>(m_data); }

    /// @returns true iff this is the empty hash.
    explicit operator bool() const { return std::any_of(m_data.begin(), m_data.end(), [](u_byte _b) { return _b != 0; }); }
    ...
    u_byte& operator[](unsigned _i) { return m_data[_i]; }
    /// @returns a particular byte from the hash.
    u_byte operator[](unsigned _i) const { return m_data[_i]; }
    ...
    u_byte* data() { return m_data.data(); }

    /// @returns a constant byte pointer to the object's data.
    u_byte const* data() const { return m_data.data(); }

    /// @returns begin iterator.
    auto begin() const -> typename std::array<u_byte, N>::const_iterator { return m_data.begin(); }

    /// @returns end iterator.
    auto end() const -> typename std::array<u_byte, N>::const_iterator { return m_data.end(); }

    /// @returns a copy of the object's data as a byte vector.
    bytes asBytes() const { return bytes(data(), data() + N); }

    /// @returns a mutable reference to the object's data as an STL array.
    std::array<u_byte, N>& asArray() { return m_data; }

    /// @returns a constant reference to the object's data as an STL array.
    std::array<u_byte, N> const& asArray() const { return m_data; }
...
private:
    std::array<u_byte, N> m_data;        ///< The binary data.
};
...
public:
    ...
    explicit SecureFixedHash(u_byte const* _d, ConstructFromPointerType _t): FixedHash<T>(_d, _t) {}
    ...
    u_byte const* data() const { return FixedHash<T>::data(); }
...
namespace std
{
    ...
    template<> struct hash<libtorrent::aux::h64>: libtorrent::aux::h64::hash {};
    template<> struct hash<libtorrent::aux::h128>: libtorrent::aux::h128::hash {};
    template<> struct hash<libtorrent::aux::h160>: libtorrent::aux::h160::hash {};
    template<> struct hash<libtorrent::aux::h256>: libtorrent::aux::h256::hash {};
    template<> struct hash<libtorrent::aux::h512>: libtorrent::aux::h512::hash {};
}
