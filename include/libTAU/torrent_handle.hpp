...
#include "libTAU/socket.hpp" // tcp::endpoint
...
#include "libTAU/address.hpp" // for address_v4 and address_v6
...
#include "libTAU/aux_/disable_warnings_pop.hpp"
	...
	struct TORRENT_EXPORT torrent_handle
	{
		...
		using status_flags_t = libTAU::status_flags_t;
		using pause_flags_t = libTAU::pause_flags_t;
		using save_resume_flags_t = libTAU::resume_data_flags_t;
		using reannounce_flags_t = libTAU::reannounce_flags_t;
#endif

		// instruct libTAU to overwrite any data that may already have been
		...
		// used to ask libTAU to send an alert once the piece has been
		...
		// piece index (``index``). libTAU will attempt to download this
		...
		// libTAU to use another list of trackers for this torrent, you can
		...
		// a function that will be called from within libTAU's context
		...
		//
		// Note that by the time you get the alert, libTAU may have cached
		// more data for the torrent, but you are guaranteed that whatever cached
		// data libTAU had by the time you called
		...
		// ``save_resume_data()`` asks libTAU to generate fast-resume data for
		// this torrent.
		//
		...
		//	   metadata (see libTAU's metadata-from-peers_ extension)
		//
		...
		//
		// It also means that if the resume data is out dated, libTAU will
		...
		// each piece in this torrent. libTAU does not keep track of
		...
		// that libTAU uses in order to prefer picking rare pieces.
		...
		// this function is not thread safe outside of libTAU's internal
		// thread (which is used to invoke plugin callbacks).
		//
		// The ``aux::torrent`` class is not only eligible for changing ABI across
		// minor versions of libTAU, its layout is also dependent on build
		// configuration. This adds additional requirements on a client to be
		// built with the exact same build configuration as libTAU itself.
		// i.e. the ``TORRENT_`` macros must match between libTAU and the
		// client builds.
...
namespace std {
	template <>
	struct hash<libTAU::torrent_handle>
	{
		std::size_t operator()(libTAU::torrent_handle const& th) const
		{
			return libTAU::hash_value(th);
		}
	};
}
