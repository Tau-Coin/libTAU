...
#include "libTAU/storage_defs.hpp" // for storage_mode_t
...
#include "libTAU/aux_/disable_warnings_pop.hpp"
		...
		// the same data can be downloaded by mistake. When libTAU sends
		// requests to a peer, and the peer doesn't send a response within a
		// certain timeout, libTAU will re-request that block. Another
		// situation when libTAU may re-request blocks is when the requests
		...
		// uploading/downloading, or it was too long ago for libTAU to
		...
		// set to true if the torrent is auto managed, i.e. libTAU is
...
namespace std {
	template <>
	struct hash<libTAU::torrent_status>
	{
		std::size_t operator()(libTAU::torrent_status const& ts) const
		{
			return libTAU::hash_value(ts.handle);
		}
	};
}
