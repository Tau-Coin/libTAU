...
namespace aux {
	...
	struct TORRENT_EXPORT settings_pack final : settings_interface
	{
		...
		enum string_types
		{
			...
			// libTAU/libTAU-version". This name will not only be used when
		...
		enum bool_types
		{
			...
			// ``close_redundant_connections`` specifies whether libTAU should
			...
			// has been paused by the auto managed feature in libTAU, i.e. the
			...
			// of redundant bytes. If this is ``false``, libTAU attempts to
			...
			// connections are disabled, libTAU will simply not make
			...
			// when this is true, libTAU will not attempt to make outgoing
			...
			// libTAU is downloading to or seeding from. This is implemented
			// using ``fcntl(F_SETLK)`` on Unix systems and by not passing in
			// ``SHARE_READ`` and ``SHARE_WRITE`` on windows. This might prevent
			// 3rd party processes from corrupting the files under libTAU's
			// feet.
...
#if TORRENT_ABI_VERSION == 1
			// ``contiguous_recv_buffer`` determines whether or not libTAU
			...
			// if false, prevents libTAU to advertise share-mode support
			support_share_mode,
			...
			// if this is true, libTAU will fall back to listening on a port
		...
		enum int_types
		{
			...
			// ``suggest_mode`` controls whether or not libTAU will send out
			...
			//   high throughput and large files. If libTAU's read cache is
			...
			// write errors occur on a torrent until libTAU will take it out
			// of the upload mode, to test if the error condition has been fixed.
			//
			// libTAU will only do this automatically for auto managed
			// torrents.
			//
			...
			// messages, this limit prevents libTAU from using too much RAM.
			max_suggest_pieces,
			...
			// ``udp_tracker_token_expiry`` is the number of seconds libTAU
			...
			// bandwidth. 0 means automatic, where libTAU opens up 20% of your
			...
			// connections libTAU will have when connecting to peers. A
			...
			//
			// This setting may be useful to make libTAU more or less
			...
			// and then to the downloading torrent. libTAU will loop over the
			...
			// libTAU from creating too many expensive http requests per
			// second. You can select a value as high as you want but keep in mind
			// that libTAU can't create parallel requests if the first request
