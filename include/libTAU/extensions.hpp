...
// OVERVIEW
//
// libTAU has a plugin interface for implementing extensions to the protocol.
...
// structures it is also quite easy to sabotage libTAU's operation.
//
// All the callbacks are always called from the libTAU network thread. In
// case portions of your plugin are called from other threads, typically the main
// thread, you cannot use any of the member functions on the internal structures
// in libTAU, since those require being called from the libTAU network
...
// libTAU thread (through a callback). If you need to send out a message
...
// specific version of libTAU. Although, in practice the internals mostly
...
// They are found in the ``<libTAU/extensions.hpp>`` header.
//
...
// -------------
//
// Since plugins are running within internal libTAU threads, one convenient
...
// not collide with any other alert. The built-in alerts in libTAU will
...
namespace libTAU {
	...
	struct TORRENT_EXPORT plugin
	{
		...
		using feature_flags_t = libTAU::feature_flags_t;
#endif
	...
	// ability to change or override the default libTAU behavior.
	...
	struct TORRENT_EXPORT peer_plugin
	{
		...
		// called when libTAU think this peer should be disconnected.
