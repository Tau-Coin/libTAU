...
namespace errors {
	// libTAU uses boost.system's ``error_code`` class to represent
	// errors. libTAU has its own error category
	// libTAU_category() with the error codes defined by
	// error_code_enum.
	enum error_code_enum
	{
		...
		// The .torrent file has more pieces than is supported by libTAU
	...
	// HTTP errors are reported in the libTAU::http_category, with error code enums in
	// the ``libTAU::errors`` namespace.
	enum http_errors
	{
		cont = 100,
		ok = 200,
	...
	// return the instance of the libTAU_error_category which
	// maps libTAU error codes to human readable error messages.
	TORRENT_EXPORT boost::system::error_category& libTAU_category();
...
#if TORRENT_ABI_VERSION == 1
	TORRENT_DEPRECATED
	inline boost::system::error_category& get_libTAU_category()
...
namespace system {
	...
	template<> struct is_error_code_enum<libTAU::errors::error_code_enum>
	{ static const bool value = true; };

	template<> struct is_error_code_enum<libTAU::errors::http_errors>
	{ static const bool value = true; };
}
}
