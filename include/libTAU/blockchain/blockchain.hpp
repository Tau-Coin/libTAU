...
//        NOTE_POOL_HASH_SET,
    ...
    struct head_block_info {
        head_block_info() = default;

        head_block_info(const sha1_hash &mHash, const dht::public_key &mPeer, uint64_t mCumulativeDifficulty) : m_hash(
                mHash), m_peer(mPeer), m_cumulative_difficulty(mCumulativeDifficulty) {}
    ...
    struct transfer_tx_info {
        transfer_tx_info() = default;

        transfer_tx_info(const sha1_hash &mHash, const dht::public_key &mPeer, int64_t mFee) : m_hash(mHash),
                m_peer(mPeer), m_fee(mFee) {}
...
//        static std::string make_salt(const aux::bytes &chain_id, std::int64_t data_type_id);
        ...
        entry select_signal_randomly(const aux::bytes &chain_id);

        void send_discovery_signal(const aux::bytes &chain_id);
