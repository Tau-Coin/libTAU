...
bool libtorrent::aux::isHex(string const& _s) noexcept
{
	auto it = _s.begin();
...
std::string libtorrent::aux::escaped(std::string const& _s, bool _all)
{
...
bytes libtorrent::aux::fromHex(std::string const& _s, WhenError _throw)
{
...
bytes libtorrent::aux::asNibbles(bytesConstRef const& _s)
{
	std::vector<uint8_t> ret;
	ret.reserve(_s.size() * 2);
	for (auto i: _s)
