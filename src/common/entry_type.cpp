...
//            m_short_chain_id = aux::bytes(chain_id.begin(), chain_id.end());
            ...
            m_hash = sha1_hash(encode.substr(5, 20).data());
            // source peer
            m_source_peer = dht::public_key(encode.substr(25, 32).data());
            // short chain id
    ...
    entry signal_entry::get_entry() {
        ...
        // source peer:32 bytes
        if (!m_source_peer.is_all_zeros()) {
            encode.append(std::string(m_source_peer.bytes.begin(), m_source_peer.bytes.end()));
        }
